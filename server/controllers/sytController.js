...
exports.submitEntry = async (req, res) => {
  try {
    ...
    // 💰 REWARD: Award coins for SYT upload with daily limit
      ...
      if (!user) {
        ...
        if (user.dailyUploadsCount < maxUploadsPerDay) {
          ...
          await Transaction.create({
            ...
            description: `SYT entry upload reward (${user.dailyUploadsCount}/${maxUploadsPerDay} today)`,
            relatedSYTEntry: entry._id,
          });

          console.log(`✅ SYT upload reward: ${uploadRewardCoins} coins added to user ${req.user.id} (${user.dailyUploadsCount}/${maxUploadsPerDay})`);
        } else {
          console.log(`⚠️ Daily upload limit reached for user ${req.user.id}`);
        }
      }
    } catch (coinError) {
      console.warn('⚠️ Error awarding SYT upload coins:', coinError.message);
...
exports.voteEntry = async (req, res) => {
  try {
    ...
    // Vote costs 1 coin and awards 1 coin to the creator
    const VOTE_COST = 1;
    const VOTE_REWARD = 1;
    ...
    if (!voter || voter.coinBalance < VOTE_COST) {
      return res.status(400).json({
        success: false,
        message: `Insufficient coins to vote. You need at least ${VOTE_COST} coin to vote.`,
    ...
    try {
      await deductCoins(
        ...
        VOTE_COST,
        'vote_cast',
        `Voted for SYT entry (-${VOTE_COST} coin)`,
        { relatedSYTEntry: entry._id }
      );
    } catch (coinError) {
    ...
    await awardCoins(
      ...
      VOTE_REWARD,
      'vote_received',
      `Vote received on SYT entry (+${VOTE_REWARD} coin)`,
      { relatedSYTEntry: entry._id }
    );
