...
class NotificationService {
  ...
  void _setupWebSocketListeners() {
    ...
    wsService.setOnNewNotification((data) {
      print('📱 New notification in service: $data');
      ...
      if (data['notification'] != null) {
        onNewNotification?.call(data['notification']);
        _showLocalNotification(data['notification']);
      }
    });

    // Listen for unread count updates
    wsService.setOnUnreadCountUpdate((count) {
      _unreadCount = count;
      onUnreadCountChanged?.call(_unreadCount);
    });

    // Listen for system notifications
    wsService.setOnSystemNotification((data) {
  ...
  void _showLocalNotification(Map<String, dynamic> notification) {
    ...
    switch (type) {
      ...
      case 'gift':
        ...
        final giftType = data['metadata']?['giftType'] ?? 'coins';
  ...
  void setOnUnreadCountChanged(Function(int) callback) {
  ...
  void updateUserStatus(String status) {
    WebSocketService.instance.updateStatus(status);
  }

  // Send typing indicator
  void sendTypingIndicator(String recipientId, bool isTyping) {
    WebSocketService.instance.sendTypingIndicator(recipientId, isTyping);
  }

  // Mark notification as read
  void markNotificationAsRead(String notificationId) {
    WebSocketService.instance.markNotificationAsRead(notificationId);
  }
