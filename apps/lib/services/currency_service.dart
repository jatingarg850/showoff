...
class CurrencyService {
  static const String
  _currencyKey = 'user_currency';
  static const String
  _ratesKey = 'exchange_rates';
  static const String
  _ratesTimestampKey = 'rates_timestamp';

  // Free exchange rate API (no key required)
  static const String
  _apiUrl = 'https://api.exchangerate-api.com/v4/latest/USD';
  ...
  getUserCurrency() async {
    try {
      final savedCurrency = await StorageService.getString(
        _currencyKey,
      );
      if (savedCurrency !=
              null &&
  ...
  getExchangeRates() async {
    try {
      // Check cache first
      final cachedRates = await StorageService.getString(
        _ratesKey,
      );
      final timestamp = await StorageService.getString(
        _ratesTimestampKey,
      );
  ...
  formatPrice(
    ...
    try {
      final convertedPrice = await convertFromUSD(
        usdPrice,
      );
  ...
  /// Get coin amount (1 local currency unit = 100 coins)
  static int
  getCoinAmount(
